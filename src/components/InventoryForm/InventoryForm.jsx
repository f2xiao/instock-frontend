...
const InventoryForm = ({ id, action, warehouseList, handleBackClick }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        const addInventory = async () => {
            try {
                const response = await axios.post(`${API_URL}/api/inventories`, inventoryDetails)
        ...
        const updateInventory = async () => {
            try {
                const response = await axios.put(`${API_URL}/api/inventories/${id}`, inventoryDetails);
                return response.data;
            } catch (error) {
        ...
        if (!hasErrors) {
            ...
            try {
                if (inventoryDetails.warehouse_id) {
                    inventoryDetails.warehouse_id = parseInt(inventoryDetails.warehouse_id);
                }
    ...
    return (
        <form className='add__form' onSubmit={handleSubmit}>
                ...
                <section className='add__section add__section1'>
                    ...
                    <div className='form-field'>
                        ...
                        <input
                            ...
                            className={formErrors.item_name ? "add__input-red add__name-input" : "add__name-input"}
                        />
                    ...
                    <div className='form-field'>
                        ...
                        <textarea
                            ...
                            className={formErrors.description ? "add__description add__input-red" : "add__description"}
                            onChange={handleInventoryChange}
                        ...
                        <div className='form-field'>
                            ...
                            <div className="add_arrow">
                                <select
                                    id="category"
                                    name="category"
                                    className={formErrors.category ? "add__category add__input-red" : "add__category"}
                                    value={inventoryDetails.category}
                                    onChange={handleInventoryChange}
                                >
                                    <option value={null} key='-1'>Please select</option>
            ...
            <div className="add__buttons">
                <button className="add__cancel" type="button" onClick={handleBackClick}><h3>Cancel</h3></button>
                <button className="add__save" type="submit"><h3>{action === 'Add' ? '+ Add item' : 'Save'}</h3></button>
            </div>
        </form>
    );
};
